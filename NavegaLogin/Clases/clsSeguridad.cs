...
using System.Text;
	...
	public class clsSeguridad
	{
        ...
        public bool ObtieneAplicaciones(string pcodusuario)
        {
             ...
             */
            sqry += "SELECT     CTL_APLICACION.CodEmpresa, CTL_APLICACION.CodAplicacion, CTL_EMPRESA.Descripcion AS NombreEmpresa,  ";
            sqry += "CTL_APLICACION.Descripcion AS NombreAplicacion, CTL_APLICACION.Version, CTL_APLICACION.Imagen, CTL_EMPRESA.CodPais, CTL_PAIS.Descripcion as Pais   ";
            sqry += "FROM         CTL_EMPRESA INNER JOIN  ";
            sqry += "CTL_APLICACION ON CTL_EMPRESA.CodEmpresa = CTL_APLICACION.CodEmpresa INNER JOIN  ";
            sqry += "CTL_ROL ON CTL_APLICACION.CodEmpresa = CTL_ROL.CodEmpresa AND CTL_APLICACION.CodAplicacion = CTL_ROL.CodAplicacion INNER JOIN  ";
            sqry += "CTL_ROL_USUARIO ON CTL_ROL.CodEmpresa = CTL_ROL_USUARIO.CodEmpresa AND  ";
            sqry += "CTL_ROL.CodAplicacion = CTL_ROL_USUARIO.CodAplicacion AND CTL_ROL.CodRol = CTL_ROL_USUARIO.CodRol  ";
            sqry += "inner join CTL_PAIS on CTL_EMPRESA.CodPais = CTL_PAIS.CodPais ";
            sqry += "WHERE      (CTL_ROL_USUARIO.CodUsuario = " + clsOperadorDB.scm(pcodusuario) + ") ";
            sqry += "ORDER BY CTL_PAIS.Orden, CTL_APLICACION.CodEmpresa, CTL_APLICACION.CodAplicacion ";
        ...
        public bool Valida(string pUsuario, string pClave)
        {
            ...
            string query = "SELECT ClaveTemporal, FechaTemporal FROM SEG_EMPLEADO_TIPOAUTENTICACION WHERE CodUsuario=" + clsOperadorDB.scm(pUsuario) + " AND CodTipoAut=1";
            ...
            if (odb.DS.Tables["SEG_EMPLEADO_TIPOAUTENTICACION"].Rows.Count > 0)
            {
                ...
                try
                {
                    fechaTemporal = Convert.ToDateTime(odb.DS.Tables["SEG_EMPLEADO_TIPOAUTENTICACION"].Rows[0]["FechaTemporal"]);
                    ...
                    passTemporal =  (Byte[])(odb.DS.Tables["SEG_EMPLEADO_TIPOAUTENTICACION"].Rows[0]["ClaveTemporal"]);
            ...
            if (odb.DS.Tables["seg"].Rows.Count > 0)
            {
                if (odb.DS.Tables["seg"].Rows[0]["activoinactivo"].ToString() == "S")
                {
                    ...
                    //string CLAVESAVE = oen.DesencriptarDatos(byteBLOBData);
                    //ViewPass(pUsuario);
        ...
        private void ViewPass(string usuario)
        {
            odb.ConsultaSql("select palabraclave, activoinactivo from ctl_empleado where codusuario=" + clsOperadorDB.scm(usuario), "imagenes");
            Byte[] byteBLOBData = new Byte[0];
            byteBLOBData = (Byte[])(odb.DS.Tables["imagenes"].Rows[0]["palabraclave"]);

            if (odb.DS.Tables["imagenes"].Rows[0]["palabraclave"].ToString() != "")
            {
                byteBLOBData = (Byte[])(odb.DS.Tables["imagenes"].Rows[0]["palabraclave"]);
                /*
                string extension = "TXT"; //odb.DS.Tables["imagenes"].Rows[0][i + 1].ToString();
                string contenido = "";
                if (extension.Length > 0)
                {
                    contenido = odb.EjecutaEscalar("select contenttype from ctl_web_extension where extension='" + extension + "'");
                }
                */
                System.IO.MemoryStream stmBLOBData = new System.IO.MemoryStream(byteBLOBData);
                string path = this.patheif.Substring(0, this.patheif.Length - 9);
                string archivo = path + "Reportes\\tmp" + usuario + DateTime.Now.ToString("HHmmss");// +"." + extension;
                System.IO.FileStream fs = new System.IO.FileStream(archivo, System.IO.FileMode.Create, System.IO.FileAccess.Write);
                stmBLOBData.WriteTo(fs);
                fs.Close();
                string CLAVESAVE = oen.DesencriptarDatos(archivo);
                //habilitar luego chango
        ...
        public void IngresoBitacora(string pCodUsuario, string pTipo, string pMensaje)
        {
            ...
            odbxb.EjecutaSql("exec SSO_INSERTABITACORA " + clsOperadorDB.scm(pCodUsuario) + "," + clsOperadorDB.scm(pTipo) + "," + clsOperadorDB.scm(pMensaje) + "," + clsOperadorDB.scm(Ip));
        }
        ...
        public bool CambioClave(string pcodusuario, string pclaveactual, string pclavenueva)
        {
            ...
            if (resultado)
            {
                if ((compruebaNivelPass(pclavenueva, pcodusuario)) && (verificarHistorialPass(pcodusuario,pclavenueva, pclaveactual)))
                {
                    ...
                    byte[] ba = ms.ToArray();
                    string sqry = "update ctl_empleado set PalabraClave=@File, FechaInicioTemporal=getDate() where codusuario=" + clsOperadorDB.scm(pcodusuario);
                    ...
                    string query = "UPDATE SEG_EMPLEADO_TIPOAUTENTICACION SET FechaTemporal = null, " +
                        "ClaveTemporal = null WHERE CodUsuario = " + clsOperadorDB.scm(pcodusuario) + " AND CodTipoAut = 1";
        ...
        public bool RestableceClave(string pUsuarioIT, string pCodUsuario, string pClave1, string pClave2)
        {
            ...
            {
                if ((compruebaNivelPass(pClave1, pCodUsuario)) && (verificarHistorialPass(pCodUsuario, pClave1, "")))
                {
                    clsOperadorDB mydb = new clsOperadorDB("seguridad");
                    mydb.CargaInfoSSO(this.patheif, "ssNavega");
        ...
        public bool Insertar()
        {
            ...
            else
            {
                
                strqry = "insert into CTL_EMPLEADO (codusuario,codpuesto,apellidos,nombres,activoinactivo,direccionelectronica,observacion,codorganizacion,Bloqueado) values(";
                ...
                strqry += clsOperadorDB.scm(this.activo) + "," + clsOperadorDB.scm(this.correo) + "," + clsOperadorDB.scm(this.observacion) + "," + clsOperadorDB.VNSQL(this.codorganizacion) + ",'N')";
                
                this.codigoerror = 0;
                this.descripcionerror = "";
        ...
        public bool bloquearUsuario(string Usuario)
        {
            ...
            
            string query = "UPDATE CTL_EMPLEADO SET Bloqueado = 'S', FechaInicioTemporal = getdate() " + //clsOperadorDB.scm(System.DateTime.Today.ToString()) +
                " WHERE CodUsuario = " + clsOperadorDB.scm(Usuario);
            
        ...
        public bool comprobarContraseña(string usuario)
        {
            ...
            query = "SELECT FechaInicioTemporal FROM CTL_EMPLEADO WHERE CodUsuario = " + clsOperadorDB.scm(usuario);
            DateTime fecha;
            try
            {
                ...
                if (dias <= 7)
                {
                    ...
                    switch (dias)
                    {
                        case 0:
                            ...
                            {
                                case 0:
                                    descripcionerror = "Su contraseña expirara en menos de una hora, " + "por favor proceda a cambiarla";
                                    break;
                                case 1:
                                    descripcionerror = "Su contraseña expirara en una hora, por favor" + "proceda a cambiarla lo más pronto posible";
                                    break;
                                default:
                                    descripcionerror = "Su contraseña expirara en " + horas + " horas, " +  "por favor proceda a cambiarla lo más pronto posible";
                                    break;
                            }//fin switch(horas)
                            break;
                        case 1:
                            descripcionerror = "Falta un día para que su contraseña expire, por favor " + "proceda a cambiarla pronto";
                            break;
                        default:
                            descripcionerror = "Faltan " + dias.ToString() + " días para que su contraseña " + "expire, por favor proceda a cambiarla pronto";
                            break;
                    }//fin switch(dias)
                }//fin if (dias <= 7)
                return false;
        ...
        public bool comprobarBloqueoUsuario(string usuario)
        {
            ...
            
            string query = "SELECT Bloqueado FROM CTL_EMPLEADO WHERE CodUsuario = " + clsOperadorDB.scm(usuario);
            
            string act = "";
            try
            ...
            if (act != "N")  // S
            {
                ...
                else
                {
                    ...
                    string query3 = "SELECT FechaTemp FROM CTL_EMPLEADO WHERE CodUsuario = " + clsOperadorDB.scm(usuario);
                    ...
                    if (tipoPeriodo.ToLower().Equals("mes"))
                    {
                        ...
                        {
                            this.IngresoBitacora(usuario, "LOGIN", "Resultado de Login: Usuario esta bloqueado temporalmente, acceso denegado");
                            return true;
                        }
                        else
                        {
        ...
        public bool quitarBloqueo(string usuario)
        {
            ...
            
            string query = "UPDATE CTL_EMPLEADO SET Bloqueado = 'N', FechaTemp = null WHERE  CodUsuario = " + clsOperadorDB.scm(usuario);
            
            try
            {
        ...
        public string generarPassTemporal(String codUsuario)
        {
            ...
            query = "UPDATE SEG_EMPLEADO_TIPOAUTENTICACION SET ClaveTemporal= @File, FechaTemporal=getDate() WHERE CodUsuario=" + clsOperadorDB.scm(codUsuario) + " AND CodTipoAut=1";
            SqlConnection con = new SqlConnection(mydb.CadenaConexion);
            con.Open();
        ...
        private bool compruebaNivelPass(string pass, string usuario)
        {
            ...
            {
                codigoerror = 99;
                descripcionerror = "La contraseña no es de la longitud deseada, Ingrese una contraseña de longitud igual o mayor a " + longPass;
                this.IngresoBitacora(usuario, "PASS", "Resultado de PASS: " + descripcionerror);
                return false;
            ...
            switch (nivelPass)
            {
                case 0://nivel basico
                    ...
                    {
                        codigoerror = 99;
                        descripcionerror = "La contraseña no contiene los caracteres minimos del nivel de seguridad Basico";
                        this.IngresoBitacora(usuario, "PASS", "Resultado de PASS: " + descripcionerror);
                        return false;
                    }
                    break;//case 0
                case 1: //nivel intermedio
                    ...
                    {
                        codigoerror = 99; 
                        descripcionerror = "La contraseña no contiene los caracteres minimos del nivel de seguridad Intermedio";
                        this.IngresoBitacora(usuario, "PASS", "Resultado de PASS: " + descripcionerror);
                        return false;
                ...
                case 2:
                    bool encontroSym = false;
                        ...
                        {
                            codigoerror = 99;
                            descripcionerror = "La contraseña no contiene los caracteres minimos del nivel de seguridad Avanzado";
                            this.IngresoBitacora(usuario, "PASS", "Resultado de PASS: " + descripcionerror);
                            return false;
        ...
        private bool verificarHistorialPass(string codUsuario, string pass, string passActual)
        {
            ...
            {
                codigoerror = 99;
                descripcionerror = "No se puede verificar los parametros para el historial, notificar al administrador";
                return false;
            }//fin try y catch
            ...
            query = "SELECT TOP 3 PalabraClave FROM SSO_HISTORIAL_PASS WHERE CodUsuario = " + clsOperadorDB.scm(codUsuario) + " ORDER BY Fecha DESC ";
            try
            {
            ...
            {                
                ...
                Byte[] passTemporal = new Byte[0];
                passTemporal = (Byte[])(odb.DS.Tables["SSO_HISTORIAL_PASS"].Rows[i]["PalabraClave"]);
